 ...
 */
export function detectHachikoPR(pr: PullRequest): HachikoPR | null {
 ...
 */
export async function getOpenHachikoPRs(
 ...
 */
export async function getClosedHachikoPRs(
  context: ContextWithRepository,
  migrationId: string,
  logger?: Logger
): Promise<HachikoPR[]> {
 ...
 */
export async function getHachikoPRs(
  context: ContextWithRepository,
  migrationId: string,
 ...
 */
export async function getAllOpenHachikoPRs(
  context: ContextWithRepository,
  logger?: Logger
): Promise<HachikoPR[]> {
  const log = logger || createLogger("pr-detection");
  ...
  try {
    ...
    log.info(
      { totalOpenPRs: allOpenPRs.data.length, hachikoOpenPRs: hachikoPRs.length },
      "Found all open Hachiko PRs"
    );
