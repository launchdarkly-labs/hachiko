 ...
 */
export async function generateMigrationDashboard(
  ...
  try {
    ...
    const migrationMeta = await Promise.all(
    ...
    const entries: MigrationDashboardEntry[] = [];
 ...
 */
export function generateDashboardMarkdown(dashboard: MigrationDashboard): string {
 ...
 */
function createEmptyDashboard(): MigrationDashboard {
  return {
    lastUpdated: new Date().toISOString(),
    totalMigrations: 0,
    pending: [],
 ...
 */
async function discoverMigrationDocuments(
  context: ContextWithRepository,
  logger?: Logger
): Promise<string[]> {
  const log = logger || createLogger("dashboard");
 ...
 */
async function getMigrationDocumentFromRepo(
  context: ContextWithRepository,
  filePath: string,
  ref: string = "main",
  logger?: Logger
 ...
 */
export async function updateDashboardInRepo(
  context: ContextWithRepository,
  dashboardPath: string = "MIGRATION_DASHBOARD.md",
  logger?: Logger
): Promise<void> {
  const log = logger || createLogger("dashboard");
