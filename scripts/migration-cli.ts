...
program
  ...
  .action(async (migrationId: string, stepId: string, agent: string, options) => {
    try {
      ...
      const reviewers = migration.reviewers || [];
      const reviewersText = reviewers.length > 0 ? reviewers.join(', ') : 'none specified';
      const migrationTitle = migration.title || `Migration ${migrationId}`;
...
6. Create a pull request with the following specifications:
   - Title: "Migration: ${migrationTitle} (Step ${stepId}/${totalSteps})"
...
## Migration Progress: Step ${stepId}/${totalSteps}
...
### Next Steps
${stepId < totalSteps ? `- Merge this PR to advance to step ${stepId + 1}` : '- This completes the migration'}
