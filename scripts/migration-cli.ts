...
program
  ...
  .action(async (migrationId: string, stepId: string, agent: string, options) => {
    try {
      ...
      if (process.env.GITHUB_ACTIONS) {
        console.error(`[DEBUG] get-agent-instructions called with:`);
        console.error(`[DEBUG]   migrationId: ${migrationId}`);
        console.error(`[DEBUG]   stepId: ${stepId}`);
        console.error(`[DEBUG]   agent: ${agent}`);
        console.error(`[DEBUG]   repository: ${options.repository}`);
      ...
      if (process.env.GITHUB_ACTIONS) {
        console.error(`[DEBUG]   migrationPath: ${migrationPath}`);
      }
      
      const parsedMigration = await parseMigrationDocument(migrationPath);
      
      if (process.env.GITHUB_ACTIONS) {
        console.error(`[DEBUG] Migration document parsed successfully`);
      }
      ...
      if (!repository) {
        if (process.env.GITHUB_ACTIONS) {
          console.error(`[DEBUG] Repository not provided and GITHUB_REPOSITORY not set`);
        }
        throw new Error("Repository name is required (set GITHUB_REPOSITORY or use --repository)");
      }
      
      if (process.env.GITHUB_ACTIONS) {
        console.error(`[DEBUG] All validation passed, generating instructions...`);
      }
...
The migration document contains the full context, requirements, and success criteria. Focus on delivering high-quality, well-tested changes that advance the migration to completion.`;
      ...
      if (process.env.GITHUB_ACTIONS) {
        console.error(`[DEBUG] Instructions generated successfully, length: ${instructions.length}`);
      }
