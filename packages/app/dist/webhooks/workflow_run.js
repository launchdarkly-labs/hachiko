...
exports.handleWorkflowRun = handleWorkflowRun;
const checks_js_1 = require("../services/checks.js");
const migrations_js_1 = require("../services/migrations.js");
const workflow_js_1 = require("../utils/workflow.js");
async function handleWorkflowRun(context, logger) {
    const { payload } = context;
    const { workflow_run: workflowRun } = payload;
    // Only process Hachiko agent workflows
    if (!(0, workflow_js_1.isHachikoWorkflow)(workflowRun)) {
    ...
    try {
        const workflowData = (0, workflow_js_1.extractHachikoWorkflowData)(workflowRun);
        if (!workflowData) {
...
async function handleSuccessfulRun(context, workflowData, logger) {
    const { planId, stepId, chunk } = workflowData;
    ...
    try {
        ...
        await (0, migrations_js_1.updateMigrationProgress)(context, planId, stepId, "awaiting-review", {
...
async function handleFailedRun(context, workflowData, workflowRun, // TODO: Type this properly
logger) {
    const { planId, stepId, chunk } = workflowData;
    logger.info({
        planId,
    ...
    try {
        ...
        await (0, migrations_js_1.updateMigrationProgress)(context, planId, stepId, "failed", {
...
async function extractFailureReason(context, workflowRun, // TODO: Type this properly
logger) {
    try {
...
async function addFailureComment(context, workflowData, workflowRun, // TODO: Type this properly
logger) {
    try {
        const { planId, stepId, chunk } = workflowData;
...
//# sourceMappingURL=workflow_run.js.map