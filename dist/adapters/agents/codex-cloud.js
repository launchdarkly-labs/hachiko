...
export class CodexCloudAdapter extends BaseAgentAdapter {
    ...
    codexConfig;
    baseUrl;
    constructor(policyConfig, codexConfig) {
        super(policyConfig);
    ...
    async validate() {
        try {
            ...
            const response = await this.makeAuthenticatedRequest("GET", `${this.baseUrl}/v1/models`, {
    ...
    async execute(input) {
        ...
        try {
            ...
            const response = await this.makeAuthenticatedRequest("POST", `${this.baseUrl}/v1/chat/completions`, {
                body: chatRequest,
                headers: this.getAuthHeaders(),
...
You have access to file operation tools. Use them to implement the requested changes systematically.`;
     ...
     */
    buildUserPrompt(input, fileContents) {
...
Use the file operation tools to implement the changes.`;
     ...
     */
    async readFileContents(files, repoPath) {
     ...
     */
    async applyFileOperations(operations, repoPath) {
        const fs = await import("node:fs/promises");
     ...
     */
    formatOutput(response, operations) {
        const parts = [];
        const choice = response.choices[0];
...
//# sourceMappingURL=codex-cloud.js.map