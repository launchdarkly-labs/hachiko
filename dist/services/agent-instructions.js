 ...
 */
export async function generateAgentInstructions(migrationContext, repositoryContext, logger) {
    ...
    try {
        ...
        log.info({ migrationId: migrationContext.id, agent: migrationContext.agent }, "Generated agent instructions");
        return instructions;
    }
    catch (error) {
 ...
 */
export async function generateAgentInstructionsFromRepo(context, migrationId, logger) {
    const log = logger || createLogger("agent-instructions");
    try {
        ...
        const repositoryContext = {
            ...
            url: context.payload.repository.html_url || `https://github.com/${context.payload.repository.owner.login}/${context.payload.repository.name}`,
        };
 ...
 */
function applyTemplateSubstitutions(template, migrationContext, repositoryContext) {
 ...
 */
function getInlineInstructionTemplate() {
 ...
 */
export async function generateAgentSpecificInstructions(agentType, migrationContext, repositoryContext, logger) {
    const log = logger || createLogger("agent-instructions");
    try {
...
function getClaudeSpecificInstructions() {
...
function getCursorSpecificInstructions() {
...
function getDevinSpecificInstructions() {
...
function getCodexSpecificInstructions() {
...
function getMockAgentInstructions() {
