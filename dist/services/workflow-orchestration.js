 ...
 */
export function parseDashboardCheckboxes(issueBody) {
    ...
    let lastInProgressMigrationId = null;
...
export async function normalizePR(params) {
    ...
    const alreadyNormalized = existingComments.some((c) => c.includes(`hachiko-track:${migrationId}`));
 ...
 */
export function updateMigrationFrontmatter(fileContent, action) {
    ...
    switch (action) {
        ...
        case "pause": {
            ...
            if (/^error:/m.test(updated)) {
                updated = updated.replace(/error:.*/, "error: Migration paused - PR closed without merging");
            }
            else {
                updated = updated.replace(/last_updated:.*/, `last_updated: ${now}\nerror: Migration paused - PR closed without merging`);
            }
            break;
        }
        case "complete": {
 ...
 */
export async function applyFrontmatterUpdate(migrationId, action, context) {
    const owner = context.payload.repository.owner.login;
    const repo = context.payload.repository.name;
 ...
 */
export async function isMigrationRunning(context) {
    const owner = context.payload.repository.owner.login;
    const repo = context.payload.repository.name;
 ...
 */
export async function hasOpenPRsForMigration(migrationId, context) {
 ...
 */
export async function getTotalSteps(migrationId, context) {
 ...
 */
export async function detectMigrationFromClosedPR(prNumber, prBranch, context) {
    const owner = context.payload.repository.owner.login;
    const repo = context.payload.repository.name;
    ...
    const pr = {
        ...
        labels: prData.labels.map((l) => (typeof l === "object" ? { name: l.name } : { name: l })),
