...
import { HachikoConfigSchema, validateHachikoConfig, } from "../config/schema.js";
 ...
 */
export async function loadHachikoConfig(context) {
    try {
        ...
        throw new ConfigurationError(`Failed to load configuration: ${error instanceof Error ? error.message : String(error)}`, { configPath: DEFAULT_CONFIG_PATH });
    }
}
/**
...
export function validateConfig(rawConfig) {
    try {
        ...
        throw new ConfigurationError(`Invalid configuration: ${error instanceof Error ? error.message : String(error)}`);
    }
}
